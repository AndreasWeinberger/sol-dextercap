...
class ImageAugmenter:
    ...
    def apply(self, image: torch.Tensor, debug: bool = False):
        ...
        for idx in torch.randperm(len(trans)):
            transform = trans[idx]
            if debug:
                print(f'> Augmenter: Applying Permutation "{transform}"')
...
class BlackBlockIdentifyDataset:
    ...
    def __getitem__(self, idx):
        ...
        if self.augment_image and self.augmenter is not None and image_block.shape[0] > 2 and image_block.shape[1] > 2:
            with torch.no_grad():
...
class EdgeDataset:
    ...
    def extract_block_(image: np.ndarray, edge_ends: np.ndarray,
        ...
        if len(image_block) == 0:
            xy_min, xy_max = fix_image_block(xy_min, xy_max, debugging)
            image_block = image[xy_min[1]:xy_max[1]+1, xy_min[0]:xy_max[0]+1]
        ...
        if do_augment and augmenter is not None:
            with torch.no_grad():
                image_block = augmenter.apply(torch.from_numpy(image_block).view(1, image_block.shape[0], image_block.shape[1]), debugging).numpy().reshape(image_block.shape)
...
def fix_image_block(xy_min, xy_max, debug: bool = False):
    if debug:
        print(f'\txy_min: {xy_min}')
    ...
    if xy_min[0] > xy_max[0]:
        ...
        xy_max[0] = xy_min[0]
        xy_min[0] = tmp
    if xy_min[1] > xy_max[1]:
        tmp = xy_max[1]
        xy_max[1] = xy_min[1]
        xy_min[1] = tmp

    if debug:
        print(f'\txy_min (fixed): {xy_min}')
        print(f'\txy_max (fixed): {xy_max}')
        # plt.imshow(image_block)
        # plt.show()
...
def test_marker(dataset_folder: str, dataset_file: str = "labels.json", block_size: int = 96, margin: int = 50, count: int = 5, augment: bool = True, train: bool = True, debug: bool = True):
    dataset = MarkerDataset(train=train, dataset_folder=dataset_folder, block_size=block_size, margin=margin, dataset_file=dataset_file,
                            max_num_markers=100, output_mask_image=True, output_line_mask_image=True, debugging=debug, augment_image=augment)
...
def test_block(dataset_folder: str, dataset_file: str = "labels.json", count: int = 5, augment: bool = True, train: bool = True, debug: bool = True):
    dataset = BlockCodeDataset(dataset_folder, dataset_file, debugging=debug, train=train, augment_image=augment)
    for i in range(count):
...
def test_edge(dataset_folder: str, dataset_file: str = "labels.json", count: int = 5, augment: bool = True, train: bool = True, debug: bool = True):
    dataset = EdgeDataset(dataset_folder, dataset_file, debugging=debug, train=train, augment_image=augment)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', default=0, type=int, required=True, help='"marker", "edge" or "block"')
    parser.add_argument('--labels', default='', type=str, required=True, help='The dataset file')
    parser.add_argument('--count', default=5, type=int, help='Number of elements to pull')
    ...
    parser.add_argument('--debug', default=True, type=int)
    ...
    match args.type:
        case 0:
            test_marker(args.dataset_folder, args.labels, args.block_size, args.margin, args.count, args.augment, args.train, args.debug)
        case 1:
            test_edge(args.dataset_folder, args.labels, args.count, args.augment, args.train, args.debug)
        case 2:
            test_block(args.dataset_folder, args.labels, args.count, args.augment, args.train, args.debug)
        case _:
            print("Invalid Type!")

    print('---')
    print(f'> Done testing {args.count} elements of type "{args.type}" - labels: "{os.path.join(args.dataset_folder,args.labels)}"\n')

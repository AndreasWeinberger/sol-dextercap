...
def leading_zeros(number: int, min_length: int):
    s = ''
    if min_length < 2:
        return str(number)
    for i in range(min_length):
        if float(number / pow(10.0, min_length - i - 1)) < 1:
            s += '0'
        else:
            s += str(number)
            break
...
def find_quads(points: np.ndarray, distance_thr: float, min_area: float, max_area: float, max_nearest_neighbors: int = 12):
    ...
    # for row in closed_points:
    #     for rest in itertools.combinations(indices[row], 4):
    #         comb = tuple(rest)
    #         candidates.add(comb)

    # find the nearest points for each point
    # indices = np.argsort(dists, axis=-1)
    # indices = indices[:,:max_nearest_neighbors]

    # for row in indices:
    #     for rest in itertools.combinations(row[1:], 3):
    #         comb = tuple(sorted((row[0], *rest)))
    #         candidates.add(comb)
    ...
    # candidates_points = np.take(points, candidates, axis=0)
    # hull = check_convex_4_points(candidates_points)
    # valid_hulls = hull[:,0] >= 0
    # candidates = candidates[valid_hulls,:]
    # hull = hull[valid_hulls,:]
    # candidates = np.take_along_axis(np.asarray(candidates), hull, axis=1)
    # print('after convex hull filter:', len(candidates))
    ...
    #     plt.plot(points[quad,0], points[quad,1])
    # plt.show()
...
def parse_patch(patch_def: List[str]):
    ...
    # print(len(marker_def), marker_def)
...
def load_label_patches(fn: str):
    ...
    return: marker_defs, blocks, patches
        ...
        **patches**: a list of block arrays
    '''
